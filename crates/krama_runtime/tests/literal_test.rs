use krama_core::Object;
use krama_runtime::test_eval_ok;

// --- Boolean Literals ---

test_eval_ok!(literal_boolean_true, "true", Object::Bool(true));

test_eval_ok!(literal_boolean_false, "false", Object::Bool(false));

// --- Null Literal ---

test_eval_ok!(literal_null, "null", Object::Null);

// --- Integer Literals ---

test_eval_ok!(literal_integer, "5", Object::Integer(5));

test_eval_ok!(
  literal_integer_with_separator,
  "1_000_000",
  Object::Integer(1000000)
);

// --- Float Literals ---

test_eval_ok!(literal_float, "5.5", Object::Float(5.5));

test_eval_ok!(
  literal_float_with_separator,
  "1_000.50",
  Object::Float(1000.50)
);

test_eval_ok!(literal_scientific_notation, "1e5", Object::Float(100000.0));

test_eval_ok!(
  literal_scientific_notation_with_decimal,
  "1.5e2",
  Object::Float(150.0)
);

test_eval_ok!(
  literal_scientific_notation_with_negative_exponent,
  "1e-2",
  Object::Float(0.01)
);

// --- String Literals & Indexing ---

test_eval_ok!(literal_string, "\"hello\"", Object::String("hello".into()));

test_eval_ok!(
  literal_string_concatenation,
  "\"hello\" + \" world\"",
  Object::String("hello world".into())
);

test_eval_ok!(
  literal_string_index,
  "const a = \"hello\"; a[0]",
  Object::String("h".into())
);

test_eval_ok!(
  literal_string_index_negative,
  "const a = \"hello\"; a[-1]",
  Object::String("o".into())
);

test_eval_ok!(
  literal_string_index_out_of_bounds,
  "const a = \"hello\"; a[5]",
  Object::Void
);
